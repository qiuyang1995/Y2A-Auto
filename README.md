...
[快速开始](#快速开始) · [功能概览](#功能概览) · [部署与运行](#部署与运行) · [配置说明](#配置说明) · [使用指南](#使用指南) · [人名映射维护](#人名与专有名词映射表维护指南) · [通知推送](#通知推送) · [CookieCloud](#cookiecloud-集成) · [安全特性](#安全特性) · [常见问题](#常见问题)
...
### 1. 为什么需要映射表？
在搬运明星直拍、K-Pop 舞台、赛事或特定领域视频时，原视频标题通常仅包含艺名或缩写（例如韩文 `스테이씨 재이` 或英文 `J`），原视频描述也往往没有中文全名。AI 大语言模型在生成 B 站标题和简介时，可能会因模型版本漂移或音译随机性产生**幻觉与命名不一致**（例如同一个人在不同视频中被误译为“张礼恩”或捏造“小戴”）。

为此，系统建立了闭环防护与映射机制：
1. **Prompt 防幻觉严格规则**：强制 AI 严格以原文给出的名称为准；若原文只有英文或艺名，直接使用该艺名或保留原名，**严禁自行推测、音译脑补真实中文姓名，严禁捏造未提及的昵称**；若提供了已知映射表，必须优先遵循映射表中的标准名称。
2. **全自动映射表维护**：AI 在生成元数据时自动提取识别出的人名/艺名条目，并记录到映射表中。
3. **闭环注入**：下次处理包含相同人名/艺名的视频时，系统会自动提取相关映射并注入到 Prompt 中，确保全系列视频译名严格统一。

---

### 2. 映射表文件位置与结构
映射表存储在配置文件目录中：
- 路径：`config/name_mapping.json`
- 由于 Docker 启动时挂载了 `./config:/app/config`，该文件在宿主机与容器内完全同步、持久化保存。在宿主机上使用任何文本编辑器均可直接修改。

文件结构示例：
```json
{
  "_comment": "人名/专有名词映射表。mappings 定义了原文名称到标准规范名称的映射。可在 mappings 中直接增删改标准名。程序在识别到新人名后会自动追加，若某条目已人工确认，可将 records 中的 manual_verified 设为 true 防止被 AI 自动更改。",
  "mappings": {
    "재이": "J",
    "J": "J",
    "장예은": "张叡恩"
  ...
  "records": {
    "재이": {
      ...
      "first_seen": "2026-09-16 18:00:00",
      "last_seen": "2026-09-16 22:20:15",
      "occurrences": 3,
      "sample_titles": [
        "스테이씨 재이 ( So Bad ) 논산 청년 페스티벌 직캠 STAYC Fancam 260911",
        "스테이씨 재이 ( Teddy Bear ) 논산 청년 페스티벌 직캠 STAYC Fancam 260911",
        "스테이씨 재이 ( ASAP ) 논산 청년 페스티벌 직캠 STAYC Fancam 260911"
      ],
      "sample_urls": [
        "https://www.youtube.com/watch?v=xV9evaeriQk"
      ],
      "task_ids": [
        "5fdd7e3f-36ea-4369-993c-1a5e6c79b35f"
      ]
    }
  }
}
...
- **`mappings`（快速编辑区）**：
  直接映射字典 `{ "原文名称/艺名": "标准规范名称" }`。用户可在此处直接修改、添加或删除名称。
- **`records`（审计追踪区）**：
  详细记录了该名称的标准名、人工确认状态（`manual_verified`）、出现频次（`occurrences`）、首次/最后出现时间、最近关联的视频原始标题（`sample_titles`）、视频链接（`sample_urls`）以及任务 ID（`task_ids`）。

---

### 3. 程序自动维护机制
- **自动提取与追加**：当 AI 处理新视频时，若识别到视频涉及特定人物或艺人，会自动将 `(原文名称 => 规范名称)` 添加到 `mappings` 中，并在 `records` 中记录对应的原视频标题与链接。
- **出现频次与来源累加**：若该人物再次出现，系统会自动增加 `occurrences` 计数，并滚动保留最近的视频标题与链接。
- **防止误覆盖**：如果某个条目的 `manual_verified` 为 `true`，后续 AI 自动运行生成时**绝不会覆盖**用户人工配置的标准名称。

---

### 4. 人工与定时审计维护指南

映射表支持结合脚本工具与 Antigravity 定时任务进行真实核对维护（杜绝大模型凭空猜测翻译，基于官方维基百科、官方 SNS 与汉字正名真实查验）：

1. **审计工具使用**：
   项目提供了 `scripts/audit_name_mapping.py` 工具：
   ```bash
   # 查看当前映射表的核验统计与健康度报告
   python3 scripts/audit_name_mapping.py --summary

   # 增量检查模式（默认）：检查尚未真实核验（manual_verified=False 或 audit_status!='verified'）的条目
   python3 scripts/audit_name_mapping.py --incremental

   # 全量检查模式：全盘列出所有映射条目的核验状态、关联视频与核对依据
   python3 scripts/audit_name_mapping.py --full
   ```

2. **锁定人工/真实核对记录**：
   在 `records` 中为每个条目提供了真实核验追踪字段：
   - `manual_verified`: 设为 `true` 防止后续被 AI 自动覆盖。
   - `audit_status`: `verified`（已真实核验）或 `pending`（待核验）。
   - `audit_evidence`: 真实核查的权威来源与依据（如“官方汉字正名 张员瑛”）。
   - `audit_timestamp`: 最近核验通过的时间戳。

3. **Antigravity 定时任务自动审计**：
   系统配置了 Antigravity 每周五晚 12 点（周六 00:00）的常驻定时审计任务：
   - 定时触发对 `config/name_mapping.json` 的增量真实性审计。
   - 提取新追加的人名条目，检索权威百科与官方资料核对真实中文名或规范艺名，修正错误并标记为 `verified`。
